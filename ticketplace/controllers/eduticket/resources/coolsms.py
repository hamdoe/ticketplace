...
import sys
# import string # moved to str in python 3
import socket
import time
import random
import os

#- python version compatible  # removed import md5
import hashlib
...
class mysock:
    ...
    def recv(self, datalen):
        ...
        while len(data) < datalen:
            chunk = self.sock.recv(datalen-len(data)).decode(encoding)
            if chunk == "":
                raise RuntimeError("socket connection broken")
            data = data + chunk
        return data
...
class sms:
    ...
    def md5(self, s):
        hash = hashlib.md5() #- python version 2.6.X upper

        hash.update(s.encode(self._charset))
        return ''.join("%02x" % v for v in hash.digest())
    ...
    def send(self):
        ...
        for i in range(len(self.msgque)):
            ...
            msg["LANGUAGE"] = "PYTHON-" + str.split(sys.version, " ")[0]
...
def tbsp_build(tbsp):
    ...
    for key in list(tbsp.keys()):
        if key == "MESSAGE":
            msgarr = str.split(tbsp[key], "\n")
            for line in msgarr:
...
def cscp_read(sock):
    ...
    version = sock.recv(CSCP_VERSION_SIZE).decode(encoding)
    bodylen = sock.recv(CSCP_BODYLEN_SIZE).decode(encoding)
    bodylen = int(bodylen)
...
def cscp_build(cscpstr, attach_dir="", attach_file=""):
    capsule = "CSCP2.0".encode(encoding)
    
    byte_cscpstr = cscpstr.encode(encoding)
    if attach_file:
        attachment = read_attachfile(attach_dir, attach_file) 

        #- body total length
        capsule += ("%8d" % (20+len(byte_cscpstr)+len(attachment))).encode(encoding)

        capsule += "ME".encode(encoding)
        capsule += ("%8d" % len(byte_cscpstr)).encode(encoding)
        capsule += byte_cscpstr

        capsule += "AT".encode(encoding)
        capsule += ("%8d" % len(attachment)).encode(encoding)
        capsule += attachment

    else:
        #- body total length
        capsule += ("%8d" % (10+len(byte_cscpstr))).encode(encoding)
