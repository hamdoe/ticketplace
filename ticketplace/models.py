...
class Content(db.Model):
    ...
    actor_change = Column(Integer, default=0, doc='배우 변경 여부')
    mininum_age = Column(Integer, doc='최소 적합 연령')
    capacity = Column(Integer, doc='좌석 수')
    ...
    inquire_number = Column(Text, doc='문의전화')
    invitation_ticket_number = Column(Text, doc='초대권 제공 수량')
    location = Column(Text, doc='장소: 서울, 경기, 기타', nullable=False)
    manager_email = Column(Text, doc='담당자 연락처')
    manager_name = Column(Text, doc='담당자 이름')
    manager_phone = Column(Text, doc='담당자 핸드폰 번호')
    ...
    status = Column(Text, doc='준비중, 판매중, 판매종료', default='준비중')
    subname = Column(Text, doc='부제목')
    tags = relationship('Tag', secondary=tag_association_table)
    teacher_ticket_number = Column(Text, doc='인솔자표 제공량 ex) 20명당 1매')
    theater_address1 = Column(Text, doc='극장 주소')
    theater_address2 = Column(Text, doc='극장 세부주소')
    theater_name = Column(Text, doc='극장 이름')
